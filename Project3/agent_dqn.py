...
class Agent_DQN(Agent):
    def __init__(self, env: gym.Env, args):
        ...
        self.policy_net = DQN(n_actions=env.action_space).to(self.device)
        self.target_net = DQN(n_actions=env.action_space).to(self.device)
    ...
    def make_action(self, observation, test=True):
        ...
        if sample > eps_threshold:
            with torch.no_grad():
                action = self.policy_net(observation.to('cuda')).max(1)[1].view(1, 1)
        else:
    ...
    def train(self):
        ...
        for episode in range(self.n_episodes):
            ...
            for t in count():
                ...
                self.replay_memory.push(state, action.to('cpu'), next_state, reward.to('cpu'))
                state = next_state
